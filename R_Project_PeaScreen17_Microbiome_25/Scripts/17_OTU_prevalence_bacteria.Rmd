---
title: "Prevalence of bacterial OTUs"
...
```{r}
x <- otu_table(phy.bac_all) %>% data.frame()

otu <- as(otu_table(phy.bac_all), "matrix")
if (!taxa_are_rows(phy.bac_all)) {
            otu <- t(otu)
            }
...
saveRDS(prevalence_rel, file = here("Data", "Interim", "Prevalence_OTUs_bac.rds"))
...
for (i in 1:101) {
nr <- nsamples(phy.bac_all) * ((i-1) / 100) 
nr <-  round(nr)
test_function <- function(x) {x >= 1}

keep <- genefilter_sample(phy.bac_all, test_function, A = nr)
core <- prune_taxa(keep, phy.bac_all)
n_tax[i] <- ntaxa(core)
