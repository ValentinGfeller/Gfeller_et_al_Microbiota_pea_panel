---
title: "Heritability of fungal community composition"
...
sample_data(phy.fun_rel)$Emergence <- sample_data(phy.fun_rel)$emergence %>% as.factor()
...
plot_ordination(phy.fun_rel, ord_PCoA_Bray, color = "Emergence") +
  gg_col_emerg()
t.pcoa1 <- plot_ordination(phy.fun_rel, ord_PCoA_Bray, justDF = TRUE, axes = 1:2)

# plot PCoA axis against wheat Emergence
...
d.herit_PCoA_1 <- tibble('CI lower' = CI.lower, Median = med,
                         'CI upper' = CI.upper) %>% 
  mutate(diversity = "Beta diversity fungi (PCo axis 1)") %>% 
...
d.herit_richness <- tibble('CI lower' = CI.lower, Median = med,
                          'CI upper' = CI.upper) %>% 
  mutate(diversity = "Alpha diversity fungi (OTU Richness)") %>% 
  select(diversity, everything())
...
d.herit_shannon <- tibble('CI lower' = CI.lower, Median = med,
                          'CI upper' = CI.upper) %>% 
  mutate(diversity = "Alpha diversity fungi (Shannon)") %>% 
  select(diversity, everything())
...
write.csv(d.herit_diversity, row.names = FALSE,
          file = here("Data", "Interim", "heritability_diversity_fungi.csv"))
