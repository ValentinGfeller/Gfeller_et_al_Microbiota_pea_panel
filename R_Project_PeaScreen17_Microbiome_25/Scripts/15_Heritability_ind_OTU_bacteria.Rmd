---
title: "Heritability of individual bacterial OTUs"
...
```{r}
# count table
tab_otu_clr <- phy.bac_clr %>% otu_table() %>% t() %>% data.frame()
tab_otu_clr[1:10, 1:5]

# sample table
tab_samp <- phy.bac_clr %>% sample_data() %>% data.frame()
tab_samp[1:10, 1:5]
```


# Calculate heritablity of individual OTUs
Codeed with inspiration from Brachi et al 2022 (https://doi.org/10.1073/pnas.2201285119)
...
## Calculate heritablity of one OTU
```{r}
## Run lmer on one OTU
data <- data.frame(Y = tab_otu_clr$bOTU1, ID = tab_samp$genotype)
...
data <- data.frame(Y = tab_otu_clr[ ,"bOTU1"], ID = tab_samp$genotype)
mod <- lmer(Y ~ tab_samp$rep + (1|ID), data)
b <- bootMer(mod, nsim = 999, FUN = var_ID, seed = 240126,
...
var_boot_ind <- function(Y, ID, cov = NULL, nsim = 999){
    if(is.null(cov)){
        ...
        b    <- bootMer(mod, nsim = nsim, FUN = var_ID, seed = 240126,
...
# saveRDS(t.var_boot,
#         here("Data", "Interim", "Bootstrap_bac_OTUs.rds"))

t.var_boot <- readRDS(here("Data", "Interim", "Bootstrap_bac_OTUs.rds"))
...
write.csv(tab.otu_bac_herit, row.names = FALSE,
          file = here("Data", "Interim", "heritability_otu_bac.csv"))
